...
export default function CreateListing() {
    ...
    const handleImageSubmit = (e) => {
      if(files.length > 0 && files.length  + formData.imageUrls.length < 7){ //if there are files and the total number of files is less than 7
        setUploading(true); 
        setImageUploadError(false); //remove any previous error
        const promises = []; //upload more than one image, more than one asynchronous behavior
        
       for(let i=0;i<files.length;i++){ //based on the length of the files array
        promises.push(storeImage(files[i])); //add new promise to the promises array
       }
        ...
        .catch((err) => {
          setImageUploadError('An error occurred while uploading the images. Please try again (2 MB max per image)');
          setUploading(false);
        });
    } else {
    ...
    const handleRemoveImage = (index) => {
        setFormData(
            {...formData, imageUrls: formData.imageUrls.filter((_, i) => i !== index)}) //filter out the image url at the index
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if(+formData.regularPrice< +formData.discountPrice) return
                    setError('Discounted price cannot be higher than the regular price')
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
          ...
          <div className='flex flex-wrap gap-6'>
            ...
            <div className='flex items-center gap-2'>
              <input
                type='number' id='regularPrice' min='50' max='10000' required className='p-3 border border-gray-300 rounded-lg' onChange={handleChange} value={formData.regularPrice}/>
        ...
        {formData.offer && ( 
            <div className='flex items-center gap-2'>
              <input
                ...
                max='10000' required  onChange={handleChange} value={formData.discountPrice} className='p-3 border border-gray-300 rounded-lg'
              />
              <div className='flex flex-col items-center'>
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <p className='text-orange-700 text-sm'>{imageUploadError && imageUploadError}</p>
          {
